...
const cardsDirectory = path.join(process.cwd(), 'cards')//中身を取ってくるディレクトリの場所
...
export async function getCardData(cardid) {
  const fullPath = path.join(cardsDirectory, `${cardid}.md`)
  const fileContents = fs.readFileSync(fullPath, 'utf8')
...
// // const processcwd = path.join(process.cwd(),'posts/card');
...
//     return{
//         params:{
//             cardid: fileName.replace(/\.md$/,''),//ここの名前だけ同じなら大丈夫
//         }
//     }
//   })
// }
